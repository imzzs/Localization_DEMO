# -*- coding: utf-8 -*-
"""
This script is used to generate simulated point cloud with given map, laser function.
"""
...
def intersection(voxel_position,sensor_position,unit_vector,voxel_size):   #unit_vector 是28800个array 针对输入的节点
    ...
    plane_x=array([1,0,0,-voxel_position[0]-0.5*voxel_size])
    plane_y=array([0,1,0,-voxel_position[1]-0.5*voxel_size])
    plane_z=array([0,0,1,-voxel_position[2]-0.5*voxel_size])
    t_x=t_distance(sensor_position,plane_x,unit_vector)
    t_y=t_distance(sensor_position,plane_y,unit_vector)
    t_z=t_distance(sensor_position,plane_z,unit_vector)
    #t_x t_y t_z 排序 (001,100,010)
...
#在这里 有一种情况没有考虑
    ...
    if  tmin<0 or tmax<0 or  tmin>tmax or tmin>100:
        ...
        return t_x,t_y,t_z,tmin,tmax   # t_x mid plane, tmin max in candidate tmax min in n.c.


def detection_range(unit_vector,t_x,t_y,t_z,tmin,tmax):
    vmask=bitarray('000')
    ...
    if t_x<tmax:
        lastmask[2]=1
    maskarray_=np.array(['001','010','100'])
    sort_array=np.array([t_x,t_y,t_z])
    sort=np.argsort(sort_array)
    maskarray=maskarray_[sort]
    masklist=[bitarray(i) for i in maskarray]
    det_range=[]
    index=childmask^vmask
    a=ba2int(index)
    det_range.append(a)
    for mask in masklist:
        childmask=childmask | mask 
        index_=childmask^vmask  #真正相交的
        if index_==index:
            continue
        else:
            det_range.append(ba2int(index_))
...
def dfs_traverse(OctreeNode,sensor_position,unit_vector):
    point=None
    narray=np.asarray
    amin=np.amin
    where=np.where
    ...
    if not OctreeNode:
        ...
        children=[OctreeNode.child0,OctreeNode.child1,OctreeNode.child2,OctreeNode.child3,OctreeNode.child4,OctreeNode.child5,OctreeNode.child6,OctreeNode.child7]
        ...
        if t[0]:
            if OctreeNode.depth==1:
                ...
                det_range=detection_range(unit_vector,t[0],t[1],t[2],t[3],t[4])
                length=len(det_range)
                for ind in det_range:
                    point=dfs_traverse(children[ind],sensor_position,unit_vector)
...
def loop_ray(unit_vector_world,sensor_position,root):
    start=time.time()
    intersected_list=[dfs_traverse(root,sensor_position,ray) for ray in unit_vector_world]
    intersected_array=np.asarray(list(filter(lambda x:x is not None,intersected_list)))
    end=time.time()
    print("time of a position is ",end-start)
